...
export default function Avatarmenu({ user }) {
  ...
  const removeItem = (name) => {
    const updatedItems = follows.filter((item) => item !== name);
  ...
  return (
    <>
      ...
      <Dialog size="sm" open={open} handler={handleOpen}>
        <DialogHeader>Danh sách bạn đã theo dõi.</DialogHeader>
        <DialogBody>
          <div className="flex flex-col">
            {follows?.length === 0 && <p className="text-center">Bạn chưa theo dõi ai!</p>}
            {follows?.map((item) => (
              <div
